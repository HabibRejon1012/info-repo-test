...
function Option({value, id, icon: Icon, isSelected, onClick }: {onClick: (item: string) => void, value: string, isSelected: boolean, id: string, icon?: () => React.ReactNode}){
    ...
    return <Pressable onPress={() => onClick(id)} className="flex-row p-3 mr-3 min-w-24 rounded-lg bg-white border-2 border-solid" style={{borderColor: isSelected? "gray": "white", backgroundColor: colors.background}}>
